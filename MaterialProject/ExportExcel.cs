...
namespace MaterialProject
{
    public partial class ExportExcel : Form
    {
        ...
        public ExportExcel(List<int> cars)
        {
            InitializeComponent();
            _cars = cars;
        }
        ...
        private void UserHome_Load(object sender, EventArgs e)
        {
            ...
            this.taskTableAdapter.Fill(this.myDBDataSet.Task);
            RefreshData();
        }
        ...
        void RefreshData(string searchString = null)
        {
            using (var carTaskAdapter = new TaskCarsTableAdapter())
            {
                ...
                dataGridView1.DataSource = taskTableAdapter.GetData().Where(x => tasks.Contains(x.ID) && (searchString == null || x.TaskNo.Contains(searchString) || x.ADNo.Contains(searchString))).ToList();
                //dataGridView2.DataSource = taskTableAdapter.GetData().Where(x => tasks.Contains(x.ID) && (searchString == null || x.ADNo.Contains(searchString)) && x.ADNo != "" && x.ADNo != null).ToList();
            }
        }
        ...
        {
            var cells = dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells;
            if (!dataGridView2.Rows.Cast<DataGridViewRow>().Any(x => x.Cells[0].Value.ToString() == cells[0].Value.ToString()))
                dataGridView2.Rows.Add(new string[] { cells[0].Value.ToString(), cells[1].Value.ToString(), cells[2].Value.ToString() });
            else
                ...
                if (saveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    using (var excel = new ExcelPackage(new System.IO.FileInfo(saveFileDialog.FileName)))
                    {
                        ...
                        using (var taskNDTsTableAdapter = new TaskNDTsTableAdapter())
                        {
                            ...
                            {
                                ids.Add(Convert.ToInt32(dataGridView2.Rows[i].Cells[0].Value));
                            }
                            var taskDtos = tasks.GetData().Where(x => ids.Contains(x.ID)).Select(x => new TaskDto { Id = x.ID, TaskNo = x.TaskNo, AdNo = x.ADNo, Remarks = x.Remarks, NDTRemark = x.NDTRemark }).ToList();
