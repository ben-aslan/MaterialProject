...
{
    public partial class UserTaskDetails : Form
    {
        string _taskNo;
        List<int> _cars;

        public UserTaskDetails(string taskNo, List<int> cars)
        {
            InitializeComponent();
            _taskNo = taskNo;
            _cars = cars;
        ...
        private void UserTaskDetails_Load(object sender, EventArgs e)
        {
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            using (var carTaskAdapter = new TaskCarsTableAdapter())
            {
                ...
                var task = new TaskTableAdapter().GetData().First(x => x.TaskNo == _taskNo && tasks.Contains(x.ID));
